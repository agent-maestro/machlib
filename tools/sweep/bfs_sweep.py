...
from .tactic_shortlist import TIER0_SAMPLE, TIER1_TACTICS  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--mode", choices=("tier0", "tier1"), required=True)
    p.add_argument("--output", type=Path, default=None,
                   help="JSONL output path (default: exploration/C239.../results_<mode>.jsonl)")
    p.add_argument("--workers", type=int, default=12,
                   help="parallel worker processes (tier1 only)")
    p.add_argument("--approved", action="store_true",
                   help="required to run tier1; signals user has reviewed Tier-0")
