...
`MachLib/SignedPIDLoop.lean` (new, reachable, 0 `sorryAx`). `(ha)` proved the PID contraction and
tracking bound; neither was about a datapath. `spidloop_tracks_exact` is: `spidloop` is built from
`List Bool` pairs by the signed adder, subtractor and truncating multiply, and the bound's per-step
error is **derived from those bits**. That is the step `SignedPILoop` took for the two-state cases,
and it is what turns a lemma into a join.
...
**Two rows are free, and the delay row is freer than the integrator.** The integrator is `ssub`
then `sadd`, exact in the signed representation. The delay row is the *identity on bit vectors* —
there is nothing there to round. So the whole per-step error is the state row's three multiplies at
`2·ulp` each, and the two rows that make the loop hard to **analyse** are exactly the two that are
free to **compute**. That was true of the integrator alone at 2×2; it is more true here.

**One price the PI case did not pay, carried in the statement.** Both PI functionals begin with
`1`, so `m2_state_only` is an *equality* and a state-row perturbation passes through untouched. The
PID functionals begin with `λᵢ(λᵢ−1)`, so the bound carries an explicit `K` with `|λᵢ(λᵢ−1)| ≤ K`
and a per-step term `K·6·ulp` rather than `6·ulp`. It cannot be normalised away: rescaling the
eigenvectors needs division, which this corpus does not have, and rescaling the measure moves the
same factor to the other side of the inequality.

### The specimen is where the two-state playbook stops transferring

`SignedPILoop` discharges its specimen at the **deadbeat** design `λ = 0`, which is the sharpest
possible instance there. It is the **worst** possible one here. `(ha)` recorded that `m3` is a norm
only when the eigenvalues are distinct and none is `0` or `1`; at `λ₁ = λ₂ = λ₃ = 0` every
functional vanishes, `m3 ≡ 0`, and the conclusion reads `0 ≤ 0`. **Ticking the vacuity box by
copying the PI specimen would have produced a bound that cannot fail, with every gate green** —
the `positive_branch_impossible` failure mode, reached by a different route.

So `spidloop_tracks_exact_quarter` is built at `λ = 1/2, −1/2, 1/4`: distinct, none of them `0` or
`1`, every modulus at most `1/2`, so the factor genuinely decays. Every gain is a real Q16.16 bit
vector (`gainAVec`, `gainBVec`, `gainCVec`), and **every fraction is expressed through `ulp` rather
than by division** — the whole design is generated by `q + q + q + q = 1` (`quarter_four`) together
with `4q² = q` (`quarter_sq_four`), where `q = qval quarterVec`.

**A detail worth keeping.** `qval gainBVec` is `4q³ + 2q`, but the characteristic-equation form of
`B` is `4q³ + 3q − 4q²`. They agree *only* because `4q² = q`, so `sval_gainB` is deliberately
stated in the subtractive form: with it the eigen identity is pure ring, like the other two. The
equation is proved additively and rearranged afterwards, because the cast lemma adds and does not
subtract.

**Two `rw` traps met, both already in `CLAUDE.md` in a neighbouring form.** `quarter_sq` rewrites
`q*q` *inside* `q*q*q`, so `quarter_cube` must come first. And four proofs needed their final
rewrite **deleted**: the numerals evaluate, so the preceding rewrite closes the goal by `rfl` and
the next tactic reports "no goals".

The `0 * t` hazard bit as predicted: the integrator and delay rows both carry one, and both shape
lemmas are written with `zero_mul`/`add_zero` rewrites rather than `mach_mpoly`, which the file
says in a comment.

Aggregator reaches **793 of 1 097**. Ledger unmoved: 23 rows, 7 open, 4 distinct, 243 axioms. The
`lake build` job count registered in the prose gate an hour earlier caught its own drift (795 →
796) on the first run, which is the point of registering it.
...
| `OneQueryLevelSet` | **reduced** | `oneQueryLevelSet_of_residue` → `OneQueryLevelSetResidue` — via `one_query_level_set_two_sided`, which gets the second tail from `FTerm.reflect` at no query cost; `q_F(sign) ≥ 2` reduces to it, NOT to `OneQueryDichotomy` |
| `OneQueryLevelSetResidue` | **open** | — (rigidity ∧ bounded-finiteness. The rigidity half is the real one: agreeing with a constant on a TAIL must force agreement off a finite set. The two-sided eventual dichotomy does not exclude `sign`, which is eventually constant on each side with *different* constants) |
