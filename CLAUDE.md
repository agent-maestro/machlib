...
material). `foundations/MachLib/` holds **1 071 `.lean` files** (755 top-level + 316 in subdirectories) /
**238 779 lines** / **7 469 theorems**, re-exported through the aggregator
...
find MachLib -name '*.lean' -not -path '*/Discovered/*' -exec grep -hcE '^ *theorem ' {} + \
  | paste -sd+ | bc                                    # 7 469
find MachLib -name '*.lean' -exec grep -hcE '^ *theorem ' {} + | paste -sd+ | bc   # 8 218
```
...
run (**765 of 1 071 modules reachable, 12 documented unreachable** as of 2026-08-29); quote it from
