...
material). `foundations/MachLib/` holds **1 063 `.lean` files** (747 top-level + 316 in subdirectories) /
**236 899 lines** / **7 407 theorems**, re-exported through the aggregator
...
find MachLib -name '*.lean' -not -path '*/Discovered/*' -exec grep -hcE '^ *theorem ' {} + \
  | paste -sd+ | bc                                    # 7 407
find MachLib -name '*.lean' -exec grep -hcE '^ *theorem ' {} + | paste -sd+ | bc   # 8 156
```
...
- **A new module must be REACHABLE from `MachLib.lean`** or it is never built and never gated.
  ...
  unreachable. `check_aggregator.sh` does a real transitive closure (**757 of 1063 reachable**).
...
Lean `v4.32.2`, branch `poly-euclid-spine`. All seven gates green (760 build jobs) at **true exit
