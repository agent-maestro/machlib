...
material). `foundations/MachLib/` holds **1 100 `.lean` files** (786 top-level + 314 in subdirectories) /
**250 351 lines** / **7 659 theorems**, re-exported through the aggregator
...
find MachLib -name '*.lean' -not -path '*/Discovered/*' -exec grep -hcE '^ *theorem ' {} + \
  | paste -sd+ | bc                                    # 7 659
find MachLib -name '*.lean' -exec grep -hcE '^ *theorem ' {} + | paste -sd+ | bc   # 8 379
```
...
- **A new module must be REACHABLE from `MachLib.lean`** or it is never built and never gated.
  ...
  unreachable. `check_aggregator.sh` does a real transitive closure (**796 of 1100 reachable**).
...
run (**796 of 1 100 modules reachable, 12 documented unreachable** as of 2026-09-07); quote it from
