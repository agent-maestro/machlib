...
material). `foundations/MachLib/` holds **1 062 `.lean` files** (746 top-level + 316 in subdirectories) /
**236 788 lines** / **7 405 theorems**, re-exported through the aggregator
...
find MachLib -name '*.lean' -not -path '*/Discovered/*' -exec grep -hcE '^ *theorem ' {} + \
  | paste -sd+ | bc                                    # 7 405
find MachLib -name '*.lean' -exec grep -hcE '^ *theorem ' {} + | paste -sd+ | bc   # 8 154
```
...
- **A new module must be REACHABLE from `MachLib.lean`** or it is never built and never gated.
  ...
  unreachable. `check_aggregator.sh` does a real transitive closure (**756 of 1062 reachable**).
...
Lean `v4.32.2`, branch `poly-euclid-spine`. All seven gates green (759 build jobs) at **true exit
