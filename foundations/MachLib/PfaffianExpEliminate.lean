...
/-- **Formal top degree of `expEliminate` is preserved** — `degreeY_top(expEliminate c G top p) =
degreeY_top p`. Both summands `lcY·chainReduce(c, D·G, p)` and `p·cTD(lcY)` have formal `degreeY_top =
degreeY_top p` (the reduce preserves it, and `lcY`, `cTD(lcY)` are `y_top`-free), so their `sub` has that
formal degree (`degreeY_sub_of_eq`) — even though the true leading coefficient *evaluates* to 0
(`expEliminate_lcY_top_eval_zero`), which is exactly the phantom the trim (B2) exploits. -/
theorem expEliminate_degreeY_top_eq {N : Nat} (c : PfaffianChain N) (G : MultiPoly N) (top : Fin N)
    (h_reltop : c.relations top = MultiPoly.mul G (MultiPoly.varY top))
    (h_Gtop : MultiPoly.degreeY top G = 0)
    (h_tri : ∀ j : Fin N, j ≠ top → MultiPoly.degreeY top (c.relations j) = 0)
    (p : MultiPoly N) :
    MultiPoly.degreeY top (expEliminate c G top p) = MultiPoly.degreeY top p := by
  have hvt : MultiPoly.degreeY top (MultiPoly.varY top) = 1 := by
    show (if top = top then (1 : Nat) else 0) = 1; simp
  have h_top : MultiPoly.degreeY top (c.relations top) = 1 := by
    rw [h_reltop, degreeY_mul' top G (MultiPoly.varY top)]; omega
  ...
  have hBdeg : MultiPoly.degreeY top
    ...
    rw [degreeY_mul' top p (chainTotalDeriv c (MultiPoly.leadingCoeffY top p))]; omega
  unfold expEliminate
  rw [degreeY_sub_of_eq top _ _ (by rw [hAdeg, hBdeg])]; exact hAdeg
