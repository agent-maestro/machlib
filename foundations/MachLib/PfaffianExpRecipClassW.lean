...
/-- **Coherence survives `chainRestrict` for witness-enriched chains.** Same
proof as `chainRestrict_isCoherentOn` but the top-freeness of lower relations
comes from `IsExpOrRecipW`'s triangular clause, not `IsExpChain` — so the depth
descent can pass coherence to the recursion over a *mixed* restricted chain. -/
theorem chainRestrict_isCoherentOn_W {N : Nat} (c : PfaffianChain (N + 1)) (a b : Real)
    (hW : IsExpOrRecipW c a b) (hcoh : c.IsCoherentOn a b) :
  ...
  have htop : MultiPoly.degreeY (⟨N, Nat.lt_succ_self N⟩ : Fin (N + 1))
    ...
    (hW ⟨i.val, Nat.lt_succ_of_lt i.isLt⟩).2 (⟨N, Nat.lt_succ_self N⟩ : Fin (N + 1)) i.isLt
