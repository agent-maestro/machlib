...
noncomputable def ALPHA_MAX : Real := (1.0 : Real)
...
theorem ggx_distribution_nonneg (n_dot_h_clamped : Real) (alpha : Real)
    (h1 : (n_dot_h_clamped >= (0.0 : Real)))
    (h2 : (n_dot_h_clamped <= ONE))
    (h3 : (alpha >= ALPHA_MIN))
    (h4 : (alpha <= ALPHA_MAX)) :
    ((ggx_distribution n_dot_h_clamped alpha) >= (0.0 : Real)) := by
...
theorem smith_schlick_geometry_in_unit_interval (n_dot_l_clamped : Real) (n_dot_v_clamped : Real) (alpha : Real)
    (h1 : (n_dot_l_clamped > (0.0 : Real)))
    (h2 : (n_dot_l_clamped <= ONE))
    (h3 : (n_dot_v_clamped > (0.0 : Real)))
    (h4 : (n_dot_v_clamped <= ONE))
    (h5 : (alpha >= ALPHA_MIN))
    (h6 : (alpha <= ALPHA_MAX)) :
    ((smith_schlick_geometry n_dot_l_clamped n_dot_v_clamped alpha) >= (0.0 : Real)) := by
...
theorem cook_torrance_specular_nonneg (distribution : Real) (fresnel : Real) (geometry : Real) (n_dot_l_clamped : Real) (n_dot_v_clamped : Real)
    (h1 : (distribution >= (0.0 : Real)))
    (h2 : (fresnel >= (0.0 : Real)))
    (h3 : (fresnel <= ONE))
    (h4 : (geometry >= (0.0 : Real)))
    (h5 : (geometry <= ONE))
    (h6 : (n_dot_l_clamped > (0.0 : Real)))
    (h7 : (n_dot_l_clamped <= ONE))
    (h8 : (n_dot_v_clamped > (0.0 : Real)))
    (h9 : (n_dot_v_clamped <= ONE)) :
    ((specular distribution fresnel geometry n_dot_l_clamped n_dot_v_clamped) >= (0.0 : Real)) := by
  unfold specular
  sorry  -- TODO: prove against MachLib foundations
