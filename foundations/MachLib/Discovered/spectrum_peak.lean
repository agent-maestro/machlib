...
theorem magnitude_squared_nonneg (real_part : Real) (imag_part : Real)
    ...
    ((magnitude_squared real_part imag_part) >= (0.0 : Real)) := by
...
noncomputable def spectrum_detect (power : Real) (noise_floor : Real) (margin : Real) : Real :=
  ((min (max (power - (margin * noise_floor)) (0.0 : Real)) (1.0 : Real)) / (min (max (power - (margin * noise_floor)) (1e-30 : Real)) (1.0 : Real)))

theorem spectrum_detect_zero_below_threshold (power : Real) (noise_floor : Real) (margin : Real)
    (h1 : (power >= (0.0 : Real)))
    (h2 : (power <= POWER_MAX))
    (h3 : (noise_floor >= (0.0 : Real)))
    (h4 : (noise_floor <= POWER_MAX))
    (h5 : (margin >= (1.0 : Real)))
    (h6 : (margin <= (1000000.0 : Real))) :
    ((spectrum_detect power noise_floor margin) >= (0.0 : Real)) := by
