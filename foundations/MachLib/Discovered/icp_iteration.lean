import MachLib.Basic
-- Generated by the EML-lang Lean backend
-- Source module: icp_iteration
-- Source file:   <private>/icp_iteration.eml
-- Verified fns:  point_to_point_residual_sq, weighted_residual, point_to_plane_residual

import MachLib.EML
import MachLib.Trig
import MachLib.Forge

open MachLib
open MachLib.Real

noncomputable def COORD_MAX : Real := (1000000.0 : Real)
noncomputable def HALF : Real := (0.5 : Real)

axiom centroid_pair (coord : Real) : Real  -- helper (axiomatised in MachLib/Discovered)

-- ── point_to_point_residual_sq ──

noncomputable def point_to_point_residual_sq (src_x : Real) (src_y : Real) (src_z : Real) (tgt_x : Real) (tgt_y : Real) (tgt_z : Real) : Real :=
  ((((src_x - tgt_x) * (src_x - tgt_x)) + ((src_y - tgt_y) * (src_y - tgt_y))) + ((src_z - tgt_z) * (src_z - tgt_z)))

theorem icp_residual_sq_nonnegative (src_x : Real) (src_y : Real) (src_z : Real) (tgt_x : Real) (tgt_y : Real) (tgt_z : Real)
    (h1 : ((abs src_x) <= COORD_MAX))
    (h2 : ((abs src_y) <= COORD_MAX))
    (h3 : ((abs src_z) <= COORD_MAX))
    (h4 : ((abs tgt_x) <= COORD_MAX))
    (h5 : ((abs tgt_y) <= COORD_MAX))
    (h6 : ((abs tgt_z) <= COORD_MAX)) :
    ((point_to_point_residual_sq src_x src_y src_z tgt_x tgt_y tgt_z) >= (0.0 : Real)) := by
  unfold point_to_point_residual_sq
  sorry  -- TODO: prove against MachLib foundations

-- ── weighted_residual ──

noncomputable def weighted_residual (residual : Real) (delta : Real) : Real :=
  (((HALF * (min (max (abs residual) (0.0 : Real)) delta)) * (min (max (abs residual) (0.0 : Real)) delta)) + (delta * ((abs residual) - (min (max (abs residual) (0.0 : Real)) delta))))

theorem huber_weighted_residual_bounded (residual : Real) (delta : Real)
    (h1 : ((abs residual) <= COORD_MAX))
    (h2 : (delta > (0.0 : Real)))
    (h3 : (delta <= COORD_MAX)) :
    ((weighted_residual residual delta) >= (0.0 : Real)) := by
  unfold weighted_residual
  sorry  -- TODO: prove against MachLib foundations

-- ── point_to_plane_residual ──

noncomputable def point_to_plane_residual (src_x : Real) (src_y : Real) (src_z : Real) (tgt_x : Real) (tgt_y : Real) (tgt_z : Real) (n_x : Real) (n_y : Real) (n_z : Real) : Real :=
  ((((src_x - tgt_x) * n_x) + ((src_y - tgt_y) * n_y)) + ((src_z - tgt_z) * n_z))

theorem point_to_plane_residual_signed (src_x : Real) (src_y : Real) (src_z : Real) (tgt_x : Real) (tgt_y : Real) (tgt_z : Real) (n_x : Real) (n_y : Real) (n_z : Real)
    (h1 : ((abs src_x) <= COORD_MAX))
    (h2 : ((abs src_y) <= COORD_MAX))
    (h3 : ((abs src_z) <= COORD_MAX))
    (h4 : ((abs tgt_x) <= COORD_MAX))
    (h5 : ((abs tgt_y) <= COORD_MAX))
    (h6 : ((abs tgt_z) <= COORD_MAX))
    (h7 : ((abs n_x) <= (1.0 : Real)))
    (h8 : ((abs n_y) <= (1.0 : Real)))
    (h9 : ((abs n_z) <= (1.0 : Real))) :
    True := by
  trivial
