import MachLib.Basic
-- Generated by the EML-lang Lean backend
-- Source module: adam_optimizer
-- Source file:   <private>/adam.eml
-- Verified fns:  first_moment_update, second_moment_update, adam_param_step, adamw_param_step

import MachLib.EML
import MachLib.Trig
import MachLib.Forge

open MachLib
open MachLib.Real

noncomputable def LR_MIN : Real := (1e-09 : Real)
noncomputable def LR_MAX : Real := (1.0 : Real)
noncomputable def BETA_MAX : Real := (0.9999999 : Real)
noncomputable def VAL_MAX : Real := (1000000.0 : Real)
noncomputable def EPS_MIN : Real := (1e-12 : Real)
noncomputable def EPS_MAX : Real := (0.001 : Real)

-- ── first_moment_update ──

noncomputable def first_moment_update (prev_m : Real) (gradient : Real) (beta1 : Real) : Real :=
  ((beta1 * prev_m) + (((1.0 : Real) - beta1) * gradient))

theorem first_moment_convex_combination (prev_m : Real) (gradient : Real) (beta1 : Real)
    (h1 : ((abs prev_m) <= VAL_MAX))
    (h2 : ((abs gradient) <= VAL_MAX))
    (h3 : (beta1 >= (0.0 : Real)))
    (h4 : (beta1 <= BETA_MAX)) :
    True := by
  trivial

-- ── second_moment_update ──

noncomputable def second_moment_update (prev_v : Real) (gradient : Real) (beta2 : Real) : Real :=
  ((beta2 * prev_v) + ((((1.0 : Real) - beta2) * gradient) * gradient))

theorem second_moment_nonneg (prev_v : Real) (gradient : Real) (beta2 : Real)
    (h1 : (prev_v >= (0.0 : Real)))
    (h2 : (prev_v <= VAL_MAX))
    (h3 : ((abs gradient) <= VAL_MAX))
    (h4 : (beta2 >= (0.0 : Real)))
    (h5 : (beta2 <= BETA_MAX)) :
    ((second_moment_update prev_v gradient beta2) >= (0.0 : Real)) := by
  unfold second_moment_update
  sorry  -- TODO: prove against MachLib foundations

-- ── adam_param_step ──

noncomputable def adam_param_step (theta : Real) (m_hat : Real) (v_hat : Real) (lr : Real) (eps : Real) : Real :=
  (theta - ((lr * m_hat) / ((Real.sqrt v_hat) + eps)))

theorem adam_param_step_descent_when_aligned (theta : Real) (m_hat : Real) (v_hat : Real) (lr : Real) (eps : Real)
    (h1 : ((abs theta) <= VAL_MAX))
    (h2 : ((abs m_hat) <= VAL_MAX))
    (h3 : (v_hat >= (0.0 : Real)))
    (h4 : (v_hat <= VAL_MAX))
    (h5 : (lr >= LR_MIN))
    (h6 : (lr <= LR_MAX))
    (h7 : (eps >= EPS_MIN))
    (h8 : (eps <= EPS_MAX)) :
    True := by
  trivial

-- ── adamw_param_step ──

noncomputable def adamw_param_step (theta : Real) (m_hat : Real) (v_hat : Real) (lr : Real) (eps : Real) (weight_decay : Real) : Real :=
  (theta - (lr * ((m_hat / ((Real.sqrt v_hat) + eps)) + (weight_decay * theta))))

theorem adamw_decoupled_decay_independent_of_grad (theta : Real) (m_hat : Real) (v_hat : Real) (lr : Real) (eps : Real) (weight_decay : Real)
    (h1 : ((abs theta) <= VAL_MAX))
    (h2 : ((abs m_hat) <= VAL_MAX))
    (h3 : (v_hat >= (0.0 : Real)))
    (h4 : (v_hat <= VAL_MAX))
    (h5 : (lr >= LR_MIN))
    (h6 : (lr <= LR_MAX))
    (h7 : (eps >= EPS_MIN))
    (h8 : (eps <= EPS_MAX))
    (h9 : (weight_decay >= (0.0 : Real)))
    (h10 : (weight_decay <= (1.0 : Real))) :
    True := by
  trivial
