import MachLib.Basic
-- Generated by the EML-lang Lean backend
-- Source module: binomial_tree
-- Source file:   <private>/binomial_tree.eml
-- Verified fns:  up_factor, down_factor, risk_neutral_prob, step_back

import MachLib.EML
import MachLib.Trig
import MachLib.Forge

open MachLib
open MachLib.Real

noncomputable def ONE : Real := (1.0 : Real)
noncomputable def TINY : Real := (1e-12 : Real)
noncomputable def VOL_MIN : Real := (1e-06 : Real)
noncomputable def VOL_MAX : Real := (5.0 : Real)
noncomputable def DT_MIN : Real := (1e-06 : Real)
noncomputable def DT_MAX : Real := (10.0 : Real)

-- ── up_factor ──

noncomputable def up_factor (vol : Real) (dt : Real) : Real :=
  (Real.exp (vol * (Real.sqrt dt)))

theorem binomial_up_factor_strictly_above_one (vol : Real) (dt : Real)
    (h1 : (vol >= VOL_MIN))
    (h2 : (vol <= VOL_MAX))
    (h3 : (dt >= DT_MIN))
    (h4 : (dt <= DT_MAX)) :
    ((up_factor vol dt) > (1.0 : Real)) := by
  unfold up_factor
  sorry  -- TODO: prove against MachLib foundations

-- ── down_factor ──

noncomputable def down_factor (vol : Real) (dt : Real) : Real :=
  (ONE / (Real.exp (vol * (Real.sqrt dt))))

theorem binomial_down_factor_below_one (vol : Real) (dt : Real)
    (h1 : (vol >= VOL_MIN))
    (h2 : (vol <= VOL_MAX))
    (h3 : (dt >= DT_MIN))
    (h4 : (dt <= DT_MAX)) :
    ((down_factor vol dt) < (1.0 : Real)) := by
  unfold down_factor
  sorry  -- TODO: prove against MachLib foundations

-- ── risk_neutral_prob ──

noncomputable def risk_neutral_prob (rate : Real) (vol : Real) (dt : Real) : Real :=
  (((Real.exp (rate * dt)) - (ONE / (Real.exp (vol * (Real.sqrt dt))))) / (((Real.exp (vol * (Real.sqrt dt))) - (ONE / (Real.exp (vol * (Real.sqrt dt))))) + TINY))

theorem risk_neutral_prob_in_unit_interval (rate : Real) (vol : Real) (dt : Real)
    (h1 : (vol >= VOL_MIN))
    (h2 : (vol <= VOL_MAX))
    (h3 : (dt >= DT_MIN))
    (h4 : (dt <= DT_MAX))
    (h5 : ((rate * dt) < (vol * (Real.sqrt dt))))
    (h6 : ((rate * dt) > ((-vol) * (Real.sqrt dt)))) :
    ((risk_neutral_prob rate vol dt) > (0.0 : Real)) := by
  unfold risk_neutral_prob
  sorry  -- TODO: prove against MachLib foundations

-- ── step_back ──

noncomputable def step_back (rate : Real) (dt : Real) (p : Real) (value_up : Real) (value_down : Real) : Real :=
  ((Real.exp ((-rate) * dt)) * ((p * value_up) + ((ONE - p) * value_down)))

theorem binomial_step_back_convex_combination (rate : Real) (dt : Real) (p : Real) (value_up : Real) (value_down : Real)
    (h1 : (p >= (0.0 : Real)))
    (h2 : (p <= (1.0 : Real)))
    (h3 : (dt >= DT_MIN))
    (h4 : (dt <= DT_MAX))
    (h5 : (value_up >= (0.0 : Real)))
    (h6 : (value_down >= (0.0 : Real))) :
    ((step_back rate dt p value_up value_down) >= (0.0 : Real)) := by
  unfold step_back
  sorry  -- TODO: prove against MachLib foundations
