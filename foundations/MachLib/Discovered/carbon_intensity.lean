...
theorem carbon_source_contribution_nonneg (usage : Real) (source_intensity : Real)
    (h1 : (usage >= (0.0 : Real)))
    (h2 : (usage <= USAGE_MAX))
    (h3 : (source_intensity >= (0.0 : Real)))
    (h4 : (source_intensity <= INTENSITY_MAX)) :
    ((source_emissions usage source_intensity) >= (0.0 : Real)) := by
