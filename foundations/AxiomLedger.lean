...
* **`primitiveByConstruction`** — a **fourth** value, added because the transcendental derivative
  seeds do not fit the third and filing them there would overstate what better tooling could buy.

  `HasDerivAt_exp` says `exp' = exp`. That is not a theorem the base could reach with a stronger
  tactic: **`exp` itself is axiomatised**, with no series and no characterisation to differentiate.
  The derivative is part of *what `exp` means here*. Same for `sin`, `cos`, `log_pos`.

  `derivableInPrinciplePinned` means *"provable, but the derivation is out of scope"*.
  `primitiveByConstruction` means **"there is nothing to derive it from"** — a stronger and more
  honest claim, and the distinction matters because only the former could ever be discharged.

  **And it states the trust boundary the analysis layer has been working toward:** the seeds are
  assumed, the calculus is proved. `MachLib/DerivDefinition.lean` defines `HasDerivAtL` precisely so
  that everything *around* the seeds derives — `const`, `id`, `of_eq`, `neg`, `add`, `sub` and `mul`
  are theorems there with **zero appeals to any `HasDerivAt` axiom**. `comp` is the last structural
  rule outstanding.
...
/-- **Primitive by construction** — there is nothing to derive these from. The function is itself
axiomatised, so its derivative is part of that axiomatisation rather than a consequence of it.
NOT `derivableInPrinciplePinned`: that category means "provable, derivation out of scope", and would
overstate what a stronger tactic could buy. -/
def primitiveByConstruction : List (Name × String) :=
  [(`MachLib.Real.HasDerivAt_exp, "exp is axiomatised; exp' = exp is part of what exp means here"),
   (`MachLib.Real.HasDerivAt_sin, "sin is axiomatised; the derivative is part of that"),
   (`MachLib.Real.HasDerivAt_cos, "cos is axiomatised; the derivative is part of that"),
   (`MachLib.Real.HasDerivAt_log_pos, "log is axiomatised; the derivative is part of that")]
...
def trustedFootprint : List Name := [`Certcom.realToR, `Certcom.real_fpbridge, `Certcom.real_tanh_rounds, `MachLib.Real.u, `MachLib.Real.u_nonneg, `MachLib.Real.mul_lt_mul_of_pos_right, `MachLib.Real.cosh, `MachLib.Real.cosh_eq, `MachLib.Real.cosh_ge_one, `MachLib.Real.cosh_pos, `MachLib.Real.exp_add, `MachLib.Real.exp_zero, `MachLib.Real.HasDerivAt_neg, `MachLib.Real.HasDerivAt_of_eq, `MachLib.Real.one_div_nonneg_of_pos, `MachLib.Real.sinh, `MachLib.Real.sinh_eq, `MachLib.Real.tanh, `MachLib.Real.tanh_eq_sinh_div_cosh, `Certcom.real_exp_rounds, `Certcom.real_log_rounds, `Certcom.real_sin_eps, `Certcom.real_sin_rounds, `Certcom.real_cos_eps, `Certcom.real_cos_rounds, `Certcom.real_atan_eps, `Certcom.real_atan_rounds, `Certcom.real_abs_eps, `Certcom.real_abs_rounds, `Certcom.real_sqrt_rounds, `Certcom.real_log10_rounds, `Certcom.real_asin_rounds, `Certcom.real_acos_rounds, `Certcom.real_sinh_rounds, `Certcom.real_cosh_rounds, `Certcom.real_tan_rounds, `MachLib.Real.sin_pos_of_pos_lt_pi_div_two, `MachLib.Real.HasDerivAt_congr, `MachLib.Real.cos_neg, `MachLib.Real.cos_pi_div_two, `MachLib.Real.pi, `MachLib.Real.tan, `MachLib.Real.tan_def, `MachLib.Real.sqrt, `MachLib.Real.sqrt_le_of_le_sq, `MachLib.Real.sqrt_nonneg, `MachLib.Real.sqrt_sq_nonneg, `MachLib.Real.arcsin, `MachLib.Real.HasDerivAt_arcsin, `MachLib.Real.arccos, `MachLib.Real.HasDerivAt_arccos, `MachLib.Real.log10, `MachLib.Real.log10_def, `MachLib.Real.atan, `MachLib.Real.HasDerivAt_atan, `MachLib.Real.HasDerivAt_cos, `MachLib.Real.sin, `MachLib.Real.cos, `MachLib.Real.HasDerivAt_sin, `MachLib.Real.pythagorean, `Classical.choice, `MachLib.IsAnalyticOnReals, `MachLib.Real, `MachLib.Real.HasDerivAt, `MachLib.Real.HasDerivAt_add, `MachLib.Real.HasDerivAt_comp, `MachLib.Real.HasDerivAt_const, `MachLib.Real.HasDerivAt_exp, `MachLib.Real.HasDerivAt_id, `MachLib.Real.HasDerivAt_inv, `MachLib.Real.HasDerivAt_log_pos, `MachLib.Real.HasDerivAt_mul, `MachLib.Real.HasDerivAt_sub, `MachLib.Real.HasDerivAt_unique, `MachLib.Real.addR, `MachLib.Real.add_assoc, `MachLib.Real.add_comm, `MachLib.Real.add_lt_add_left, `MachLib.Real.add_neg, `MachLib.Real.add_zero, `MachLib.Real.divR, `MachLib.Real.div_def, `MachLib.Real.exp, `MachLib.Real.exp_lt, `MachLib.Real.exp_pos, `MachLib.Real.exp_surj, `MachLib.Real.leR, `MachLib.Real.le_iff_lt_or_eq, `MachLib.Real.ltR, `MachLib.Real.lt_irrefl_ax, `MachLib.Real.lt_total, `MachLib.Real.lt_trans_ax, `MachLib.Real.mulR, `MachLib.Real.mul_assoc, `MachLib.Real.mul_comm, `MachLib.Real.mul_distrib, `MachLib.Real.mul_inv, `MachLib.Real.mul_one_ax, `MachLib.Real.mul_pos, `MachLib.Real.natCast, `MachLib.Real.natCast_succ, `MachLib.Real.natCast_zero, `MachLib.Real.negR, `MachLib.Real.oneR, `MachLib.Real.one_div_pos_of_pos, `MachLib.Real.rolle_ct, `MachLib.Real.subR, `MachLib.Real.sub_def, `MachLib.Real.zeroR, `MachLib.Real.zero_lt_one_ax, `MachLib.Real.zero_ne_one_ax, `MachLib.analytic_add, `MachLib.analytic_comp, `MachLib.analytic_const, `MachLib.analytic_exp, `MachLib.analytic_id, `MachLib.analytic_log_pos, `MachLib.analytic_mul, `MachLib.analytic_one_div_pos, `MachLib.analytic_sub, `Quot.sound, `propext,
  ...
  -- extraction needs alongside `sqrt_sq_nonneg`, already trusted), and
  -- `HasDerivAt_of_eps_delta` (the genuine ε-δ-to-`HasDerivAt` bridge axiom the whole Leibniz
...
def disclosedTrusted : List (Name × String) := [(`Certcom.realToR, "IEEE-754 denotation Float→Real (Float opaque, no in-Lean semantics)"), (`Certcom.real_fpbridge, "standard model as binary64 satisfies it (FPBridgeFinite): +, -, * correctly rounded (rel err ≤ u) wherever the float result is finite and, for *, the exact product is 0 or at least DBL_MIN; negation of a finite float exact — un-witnessable in Lean; Flocq-scale to ground further. RESTATED 2026-09-14 from a measurement (tools/float_bridge/measure.py): as stated before (FPBridge, every pair of floats) it failed at 87 160 of 1 647 378 finite operations, every one a product below DBL_MIN; restated it holds at 1 419 906 (max 0.9999989u). Grounded theorems take its side conditions as the hypothesis FloatSafe"), (`Certcom.real_tanh_rounds, "libm model: runtime tanh (exp-composed: x itself for abs x <= 1.3538603431225864e-8, else copysign((1-t)/(1+t), x) with t = exp(-2 abs x)) within 2u of Real.tanh at every FINITE float. Restated 2026-09-14 from a measurement (tools/float_bridge/measure.py: the runtime floats, checked against Lean's own evaluation, against a 256-bit tanh): the statement before it, within u for abs x <= R, was false (max error 1.4996u), and its R constrained nothing; against 2u no input violates it -- un-witnessable in Lean"), (`Certcom.real_exp_rounds, "libm model: runtime exp within 2u*exp hi of Real.exp, for finite x <= hi whose float exp is finite and whose exact exp is at least DBL_MIN -- un-witnessable in Lean. RESTATED 2026-09-14 from a measurement (tools/float_bridge/measure.py): the statement before (u*exp hi for every x <= hi) failed on subnormal results, up to 2^53 times its bound, and with those excluded still at 5 results just above a power of two (glibc exp bounds its own error at about 0.509 ulp, up to about 1.018u relative); restated it holds at 780 992 inputs (max 1.0028u)"), (`Certcom.real_log_rounds, "libm model: runtime log within 2u*(abs (log lo) + abs (log hi)) of Real.log, for finite x with 0 < lo <= x <= hi -- un-witnessable in Lean. RESTATED 2026-09-14 from a measurement (tools/float_bridge/measure.py): within u*(...) it failed at 20 inputs whose result lies just above a power of two (max 1.0162u; glibc's log source bounds its error at about 0.52 ulp, about 1.04u relative), inputs the harness had not tried before; and it admitted non-finite inputs, now excluded"), (`Certcom.real_sin_eps, "libm sin rounding constant (residual primitive trust)"), (`Certcom.real_sin_rounds, "libm model: runtime sin within real_sin_eps of Real.sin at every FINITE float — un-witnessable in Lean. Narrowed to finite inputs 2026-09-14: over every float it constrained realToR of infinities and NaNs, where it has no value"), (`Certcom.real_cos_eps, "libm cos rounding constant (residual primitive trust)"), (`Certcom.real_cos_rounds, "libm model: runtime cos within real_cos_eps of Real.cos at every FINITE float — un-witnessable in Lean. Narrowed to finite inputs 2026-09-14, as real_sin_rounds was"), (`Certcom.real_atan_eps, "libm atan rounding constant (residual primitive trust)"), (`Certcom.real_atan_rounds, "libm model: runtime atan within real_atan_eps of Real.atan at every FINITE float — un-witnessable in Lean. Narrowed to finite inputs 2026-09-14, as real_sin_rounds was; unrestricted, atan of +inf is the finite pi/2 and forced this constant to about 3pi/4 or more"), (`Certcom.real_abs_eps, "libm abs rounding constant (residual primitive trust)"), (`Certcom.real_abs_rounds, "libm model: runtime abs within real_abs_eps of Real.abs at every FINITE float — un-witnessable in Lean. Narrowed to finite inputs 2026-09-14, before real_abs_eps_eq_zero was added; unrestricted, fabs of +inf is +inf and forced this constant to at least 2"), (`Certcom.real_sqrt_rounds, "libm model: runtime sqrt within u*sqrt hi of Real.sqrt, for finite x with 0 <= x <= hi (finite since 2026-09-14) -- un-witnessable in Lean. NB IEEE-754 REQUIRES sqrt to be correctly rounded, unlike every transcendental here, so this is the one row whose bound is a standards guarantee rather than a libm-quality measurement"), (`Certcom.real_log10_rounds, "libm model: runtime log10 (libm log10 since 2026-09-13; it was an ln quotient, which emitted C never computed) within 3u*(abs (log10 lo) + abs (log10 hi)) of Real.log10, for finite x with 0 < lo <= x <= hi -- un-witnessable in Lean. RESTATED 2026-09-14 from a measurement (tools/float_bridge/measure.py): within u*(...) it failed at 42 194 of 644 003 inputs (max 2.076u, near x = 1); restated it holds at all 644 003; an error analysis of glibc's fdlibm log10 gives at most about 2.63u"), (`Certcom.real_asin_rounds, "libm model: runtime asin within u*(pi/2) of Real.arcsin, for finite x with abs x <= R < 1 (finite since 2026-09-14) -- un-witnessable in Lean"), (`Certcom.real_acos_rounds, "libm model: runtime acos within u*pi of Real.arccos, for finite x with abs x <= R < 1 (finite since 2026-09-14) -- un-witnessable in Lean"), (`Certcom.real_sinh_rounds, "libm model: runtime sinh (exp-composite) within 4u*cosh R of Real.sinh, for abs x <= R and a finite float result -- un-witnessable in Lean. RESTATED 2026-09-14 from a measurement (tools/float_bridge/measure.py): within u*cosh R it failed at 63 940 of 750 768 inputs, in both branches (max 2.975u near x = 709.78); restated it holds at all 750 768; glibc's exp bound puts the large branch under about 3.04u"), (`Certcom.real_cosh_rounds, "libm model: runtime cosh (exp-composite) within 4u*cosh R of Real.cosh, for abs x <= R and a finite float result -- un-witnessable in Lean. RESTATED 2026-09-14 from a measurement (tools/float_bridge/measure.py): within u*cosh R it failed at 86 920 of 750 768 inputs, in both branches (max 2.975u near x = 709.78); restated it holds at all 750 768; glibc's exp bound puts the large branch under about 3.04u"), (`Certcom.real_tan_rounds, "libm model: runtime tan within 2u*tan R of Real.tan, for finite x with abs x <= R and 0 <= R < pi/2 -- un-witnessable in Lean. RESTATED 2026-09-14 from a measurement (tools/float_bridge/measure.py): within u*tan R it failed at 138 of 550 203 inputs (max 1.0426u); restated it holds at all 550 203; glibc's tan source reports about 0.62 ulp by random sampling, about 1.24u relative"), (`MachLib.Real.sin_pos_of_pos_lt_pi_div_two, "foundational trig fact (sin positive on (0,pi/2)) -- MachLib.Real only axiomatizes sin/cos at isolated points; needed to ground cos positivity/monotonicity for tan Lipschitz bound; user-approved 2026-07-17 via AskUserQuestion, the sole new axiom in this whole 14-primitive libm-grounding arc besides the per-primitive rounding constants"), (`Certcom.floatOfR, "the Float a standard real->float quantization (e.g. round-to-nearest) assigns to a real x -- opaque, like realToR, in the OPPOSITE direction: Certcom's own machinery never needed Real->Float (it compiles EML to C and runs on floats), this direction exists only because Option D's non-approximation theorems quantify over ALL reals in an interval, not just Float-representable ones; user-directed 2026-07-22 ('do both please' / 'proceed into that') to complete the parallel with realToR/real_fpbridge rather than leave it an abstract per-theorem hypothesis"), (`Certcom.real_round_bounds, "the disclosed real->Float quantization model, DOMAIN-RESTRICTED: for x with abs x <= M <= DBL_MAX (0 <= M, both caller-supplied) that is 0 or at least DBL_MIN in magnitude, floatOfR x read back through realToR lands within u*M of x -- the standard IEEE-754 round-to-nearest-even model (relative error <= half a ULP, i.e. <= u, uniformized to the range M the same way exp_lip_local's L:=exp hi uniformizes a per-point Lipschitz bound over an interval). External referent: true of round-to-nearest-even for reals within the representable range implied by M -- NOT claimed unconditionally over all reals (outside a bounded range round-to-nearest either loses this relative-error guarantee or overflows, and no fixed bound holds). Fixed 2026-07-22 same-day after external review caught the first version (unconditional, a single fixed real_round_eps constant independent of magnitude) as false of any real rounding implementation -- disclosed axioms are unfalsifiable inside Lean, so an imprecise statement would have meant 'sorryAx-free' stopped meaning what it appears to mean; un-witnessable in Lean (Float opaque), same status as real_fpbridge. HISTORICAL NOTE, resolved: this row once flagged real_exp_rounds/real_log_rounds as carrying the same unconditional overclaim and deferred the fix. They were in fact re-stated domain-restricted on the SAME day, along with eight others -- see the retroactive audit note below this definition. All ten dropped their named eps constants in favour of u-relative bounds, which is why no real_exp_eps/real_log_eps/etc exists in the corpus; the strings above describe the axioms as they now stand; RESTATED 2026-09-14 from a measurement (tools/float_bridge/measure.py): it now needs M <= DBL_MAX and x = 0 or abs x >= DBL_MIN, because round-to-nearest of a nonzero real below DBL_MIN misses u*abs x (the statement before failed at 42 467 of 241 297 reals, up to 2^53 times) and a real far above DBL_MAX overflows; restated it holds at 167 118 (max 0.99995u)"), (`Certcom.real_fpfinite, "IEEE-754 round-to-nearest overflow rule at realToR (FPFiniteOfRange): for finite floats a and b, the float a + b, a - b or a * b is finite whenever the exact real result is at most DBL_MAX in magnitude, and negating a finite float gives a finite float -- un-witnessable in Lean (Float opaque). Owner-approved and ADDED 2026-09-14 after a measurement (tools/float_bridge/measure.py): no violation at 2 299 615 operations over 629 091 pairs of finite doubles, including exact results at DBL_MAX, at the overflow tie DBL_MAX + 2^970 and between them; the range widened to include the tie fails at 6 046 operations, every one exactly at the tie, and the rule with no range fails at 211 671. It lets FloatSafe's finiteness half be discharged from a bound on the inputs"), (`Certcom.real_round_finite, "IEEE-754 round-to-nearest-even of a real in range is finite: floatOfR x is a finite float for every real x with abs x <= DBL_MAX -- un-witnessable in Lean (Float opaque). Owner-approved and ADDED 2026-09-14 after a measurement (tools/float_bridge/measure.py): no violation at 474 186 reals (256-bit mantissas across the range, both signs, and reals at, around and between DBL_MAX and the tie DBL_MAX + 2^970); the range widened to include the tie fails at exactly the two reals at plus and minus the tie, and no range fails at 41 412"), (`MachLib.Real.u_lt_one, "the unit roundoff u is strictly below 1. u stands for binary64's 2^-53, and 2^-53 < 1; u_le_one allowed u = 1, where a rounding within u may round any real to 0, so no float's real value could be shown nonzero. Owner-approved and ADDED 2026-09-14; witnessed against Mathlib in monogate-lean with u interpreted as 1/2^53. It gives no bound on 1 - u, so no positive lower bound on any float's real value follows from it"), (`Certcom.float_lit_1_5, "the Float literal 1.5 (the proportional gain of pidRawEML) is floatOfR 1.5, the correctly rounded double of the real 1.5, so real_round_bounds and real_round_finite give its value and finiteness -- un-witnessable in Lean (Float opaque). Owner-approved and ADDED 2026-09-14 after a measurement (tools/float_bridge/measure.py): Lean evaluates it to 0x3FF8000000000000, in literal syntax and inside pidRawEML, which is 1.5 exactly. NARROW ON PURPOSE: Lean's Float.ofScientific truncates to 64 bits before rounding once, and the generic statement that every decimal literal is floatOfR of its decimal fails at 81 210 of 631 326 decimals (0.05109 among them); the registry keeps it as a control that must fail"), (`Certcom.float_lit_0_4, "the Float literal 0.4 (the integral gain of pidRawEML) is floatOfR 0.4, the correctly rounded double of the real 0.4 -- un-witnessable in Lean. Owner-approved and ADDED 2026-09-14 after a measurement: Lean evaluates it to 0x3FD999999999999A, in literal syntax and inside pidRawEML, which is round-to-nearest-even of 0.4 in exact arithmetic (0.1 ulp from the nearest midpoint). Narrow on purpose, as float_lit_1_5 says"), (`Certcom.float_lit_0_05, "the Float literal 0.05 (the derivative gain of pidRawEML) is floatOfR 0.05, the correctly rounded double of the real 0.05 -- un-witnessable in Lean. Owner-approved and ADDED 2026-09-14 after a measurement: Lean evaluates it to 0x3FA999999999999A, in literal syntax and inside pidRawEML, which is round-to-nearest-even of 0.05 in exact arithmetic (0.1 ulp from the nearest midpoint). Narrow on purpose, as float_lit_1_5 says"), (`Certcom.real_exp_finite, "the runtime exp (glibc) of a finite float at most 709 is finite -- un-witnessable in Lean. Owner-approved and ADDED 2026-09-14 after a measurement (tools/float_bridge/measure.py): no non-finite result at the 901 678 admitted doubles of 941 513, up to 709 itself; the bound widened to 710 fails at 11 037 inputs, every one at or above 709.7827128933841, so 709 sits 0.78 inside the measured overflow boundary"), (`Certcom.real_sinh_finite, "the runtime sinh (the exp-composite) of a finite float at most 710 in magnitude is finite -- un-witnessable in Lean. Owner-approved and ADDED 2026-09-14 after a measurement: no non-finite result at the 653 480 admitted doubles of 779 473, up to 710 itself; the bound widened to 711 fails at 12 172 inputs, every one at |x| >= 710.475860073944, so 710 sits 0.47 inside the measured overflow boundary"), (`Certcom.real_cosh_finite, "the runtime cosh (the exp-composite) of a finite float at most 710 in magnitude is finite -- un-witnessable in Lean. Owner-approved and ADDED 2026-09-14 after a measurement: no non-finite result at the 653 480 admitted doubles of 779 473, up to 710 itself; the bound widened to 711 fails at 12 172 inputs, every one at |x| >= 710.475860073944"), (`Certcom.real_log_finite, "the runtime log (glibc) of a finite positive float is finite -- un-witnessable in Lean. Owner-approved and ADDED 2026-09-14 after a measurement: no non-finite result at the 378 478 admitted doubles of 428 492, down to 5e-324; with 0 <= x in place of 0 < x it fails at 4 inputs, every one a zero"), (`MachLib.Real.u_le_half, "the unit roundoff u satisfies u + u <= 1, that is u <= 1/2. u stands for binary64's 2^-53. u_lt_one made 1 - u positive and said nothing about its size, so no float read back from a nonzero real had a positive lower bound and no product could be shown to be at least DBL_MIN. Owner-approved and ADDED 2026-09-14; witnessed against Mathlib in monogate-lean with u interpreted as 1/2^53"), (`MachLib.Real.u_le_inv_two_pow_52, "the unit roundoff u is at most 2^-52, stated u <= 1 / natCast (2 ^ 52). u stands for binary64's 2^-53, so it holds with a factor 2 to spare. u + u <= 1 left 4u as large as 2, so a runtime cosh read back through real_cosh_rounds had no positive lower bound; this closes that for pid_log_cosh_grounded. It does NOT make eml_var_var_certcom_witness_grounded instantiable: that certificate's own hypotheses force B above 15.5 pi, where u * exp B is about 1.5e5 at u = 2^-53 itself. Owner-approved and ADDED 2026-09-14; witnessed against Mathlib in monogate-lean with u interpreted as 1/2^53"), (`Certcom.real_abs_eps_eq_zero, "the runtime abs is exact: real_abs_eps = 0. glibc fabs clears the sign bit and nothing else (IEEE-754 5.5.1), so with real_abs_rounds a finite float's absolute value reads back exactly -- un-witnessable in Lean (Float opaque). Owner-approved and ADDED 2026-09-14 after a measurement (tools/float_bridge/measure.py): the largest error over the finite doubles real_abs_rounds is measured on is exactly 0 and every result is its input with the sign bit cleared, checked against Lean's own evaluation; a control asking the same of sin fails. It could only be added after real_abs_rounds was narrowed to finite inputs the same day: unrestricted, fabs of +inf is +inf, and with the unrestricted log, asin and acos axioms that has no reading (MachLib/FloatBridgeNonFinite.lean)")]
...
def headlines : List Name := [`MachLib.KhovanskiiConcrete.eexp_barrier_zero_count_le_47, `MachLib.eml_eval_boundedZeros, `MachLib.IterExpDepthN.chainN_khovanskii_bound_unconditional, `MachLib.IterExpDepthN.chainN_khovanskii_bound_explicit, `Certcom.pipeline_det_grounded, `Certcom.pipeline_arith_grounded, `Certcom.pid_grounded, `Certcom.pid_tanh_grounded, `Certcom.pid_exp_grounded, `Certcom.pid_log_grounded, `Certcom.pid_sin_grounded, `Certcom.pid_cos_grounded, `Certcom.pid_atan_grounded, `Certcom.pid_abs_grounded, `Certcom.pid_sqrt_grounded, `Certcom.pid_log10_grounded, `Certcom.pid_asin_grounded, `Certcom.pid_acos_grounded, `Certcom.pid_sinh_grounded, `Certcom.pid_cosh_grounded, `Certcom.pid_tan_grounded, `Certcom.pid_log_cosh_grounded, `Certcom.pid_tanhVar_grounded, `Certcom.tanhVar_controller_tracking,
  ...
  -- Added 2026-07-25: the √π project's finish line -- `∫₀^∞exp(-t²)dt = √π/2`, proven end to end
  -- in MachLib's Mathlib-free real-analysis base via a Laplace/Feynman parameter-differentiation
  ...
  -- Added 2026-09-14: the grounded certificates instantiated on the range axioms (FloatSafeInstances.lean), and their
  -- specimens. Pinned so that `real_fpfinite`, `real_round_finite` and `u_lt_one` are load-bearing in a headline, which
  -- `disclosedTrusted` requires, and so a footprint growing past the trusted set fails here.
  `Certcom.pipeline_det_grounded_instantiated, `Certcom.pipeline_arith_grounded_sum_instantiated,
  `Certcom.pipeline_det_grounded_specimen, `Certcom.pipeline_arith_grounded_sum_specimen,
  -- Added 2026-09-14: the grounded certificates instantiated on the literal, libm-finiteness and `u_le_half` axioms
  -- (GroundedPIDInstances.lean, GroundedEMLInstances.lean), and their specimens. Pinned so each of those axioms is
  -- load-bearing in a headline, which `disclosedTrusted` requires, and so a footprint growing past the trusted set fails.
  `Certcom.pid_grounded_instantiated, `Certcom.pid_tanh_grounded_instantiated, `Certcom.pid_sin_grounded_instantiated,
  `Certcom.pid_cos_grounded_instantiated, `Certcom.pid_atan_grounded_instantiated, `Certcom.pid_abs_grounded_instantiated,
  `Certcom.pid_exp_grounded_instantiated, `Certcom.pid_log_grounded_instantiated, `Certcom.pid_sqrt_grounded_instantiated,
  `Certcom.pid_log10_grounded_instantiated, `Certcom.pid_asin_grounded_instantiated,
  `Certcom.pid_acos_grounded_instantiated, `Certcom.pid_tan_grounded_instantiated,
  `Certcom.pid_sinh_grounded_instantiated, `Certcom.pid_cosh_grounded_instantiated,
  `Certcom.pid_grounded_specimen, `Certcom.pid_tanh_grounded_specimen, `Certcom.pid_sin_grounded_specimen,
  `Certcom.pid_cos_grounded_specimen, `Certcom.pid_atan_grounded_specimen, `Certcom.pid_abs_grounded_specimen,
  `Certcom.pid_exp_grounded_specimen, `Certcom.pid_log_grounded_specimen, `Certcom.pid_sqrt_grounded_specimen,
  `Certcom.pid_log10_grounded_specimen, `Certcom.pid_asin_grounded_specimen, `Certcom.pid_acos_grounded_specimen,
  `Certcom.pid_tan_grounded_specimen, `Certcom.pid_sinh_grounded_specimen, `Certcom.pid_cosh_grounded_specimen,
  `Certcom.pipeline_det_grounded_nonzero_specimen,
  `Certcom.eml_tree_grounded_eml_var_var_instantiated, `Certcom.eml_tree_grounded_depth2_instantiated,
  `Certcom.eml_tree_grounded_eml_const_one_instantiated, `Certcom.eml_var_var_pipeline_uniform_instantiated,
  `Certcom.eml_var_var_quantized_pointwise_instantiated,
  `Certcom.eml_tree_grounded_eml_var_var_specimen, `Certcom.eml_tree_grounded_depth2_specimen,
  `Certcom.eml_tree_grounded_eml_const_one_specimen, `Certcom.eml_var_var_pipeline_uniform_specimen,
  `Certcom.eml_var_var_quantized_pointwise_specimen,
  -- Added 2026-09-14, on `u_le_inv_two_pow_52` and `real_abs_eps_eq_zero` (owner-approved): the log-cosh certificate
  -- (GroundedLogCoshInstance.lean) and the four LibmBudget restatements (GroundedBudgetInstances.lean), instantiated,
  -- and their specimens. Pinned so both axioms are load-bearing in a headline, which `disclosedTrusted` requires.
  `Certcom.pid_log_cosh_grounded_instantiated, `Certcom.pid_log_cosh_grounded_specimen,
  `Certcom.pid_sin_at_budget_instantiated, `Certcom.pid_cos_at_budget_instantiated,
  `Certcom.pid_atan_at_budget_instantiated, `Certcom.pid_abs_at_budget_instantiated,
  `Certcom.pid_sin_at_budget_specimen, `Certcom.pid_cos_at_budget_specimen,
  `Certcom.pid_atan_at_budget_specimen, `Certcom.pid_abs_at_budget_specimen]

/-- **Compiler backend artifacts are not trust assumptions**, and `.axiomInfo` does not distinguish.

Found 2026-07-29 by the v4.23.0 bump: **10 of the 252 "pinned axioms" were lambda-lifted fields of
structure-instance `def`s** — `intModel._elambda_1 : Int → Int`, `emlEmptyChain._elambda_2 : Fin 0 →
Real → Real`, and friends — which the code generator emits as **genuinely `.axiomInfo` constants**.
The prefix filter below swept them in, and the headline "252 axioms pinned" counted them alongside
real trust assumptions for as long as they had been there.

The evidence that they were noise, gathered before the count was touched:

* all 10 were `.axiomInfo` at v4.20.1 (observed directly in a `git worktree` rebuild, not inferred);
* their types are code-generation shapes, not mathematics;
* **zero of the 57 headline footprints ever cited one**, across all six frozen snapshots;
* **zero real declarations cite one** — all 17 citers are `_cstage1`/`_cstage2`/`_closed_N._cstage2`;
* v4.23.0's compiler stopped emitting them, which is what made the rot check fire.

So the honest count of user-declared trust assumptions is **242**, and always was. Filtering here makes
the miscount **unrepresentable** rather than merely corrected — deleting ten list entries would have
left the next compiler-emitted axiom-shaped closure free to walk back in. -/
...
def liveAxioms (env : Environment) : Array Name := Id.run do
  ...
  for (nm, ci) in env.constants.toList do
    if (ci matches .axiomInfo _) && !(isCompilerArtifact nm)
        && ((`MachLib).isPrefixOf nm || (`Real).isPrefixOf nm || (`Certcom).isPrefixOf nm) then
      r := r.push nm
  return r
...
def algebraSpineModules : List Name := [
  ...
  `MachLib.PolyRingLaws,
  `MachLib.PolyDivIdentity,
  `MachLib.PolyDvd,
  `MachLib.PolyDvdAlgebra,
  `MachLib.PolyPEq,
  `MachLib.PolyBezout,
  `MachLib.PolyGcd,
  `MachLib.PolyIrred,
  ...
  `MachLib.PolyOrd,
  `MachLib.PolyDeriv,
  `MachLib.PolyPowDeriv,
  `MachLib.PolyPoleOrder,
  `MachLib.PolyPoleCount,
  `MachLib.PolyConstDvd,
  `MachLib.PolyDerivShort,
  `MachLib.PolyNsum,
