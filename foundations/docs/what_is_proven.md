...
### The derivative term at the datapath

`spidloop_tracks_exact` (`MachLib/SignedPIDLoop.lean`) instantiates all of the above at bits — the
step that turns a lemma into a join, and the one `SignedPILoop` took for the two-state cases:
...
**Two of the three rows are free, and the delay row is freer than the integrator.** The integrator
is `ssub` then `sadd`, both exact in the signed representation. The delay row is the *identity on
bit vectors*, so there is nothing there to round at all. The whole per-step error is the state
row's three multiplies at `2·ulp` each. The two rows that make the loop hard to **analyse** are
exactly the two that are free to **compute**.

**One price the PI case did not pay.** Both PI functionals begin with `1`, so a state-row
perturbation passes through the measure untouched. The PID functionals begin with `λᵢ(λᵢ−1)`, so
the bound carries an explicit `K` with `|λᵢ(λᵢ−1)| ≤ K` and a per-step term `K·6·ulp`. It cannot be
normalised away: rescaling the eigenvectors needs division, and rescaling the measure only moves
the same factor to the other side of the inequality.

**The specimen is not the deadbeat design, and it could not be.** `SignedPILoop` discharges its
specimen at `λ = 0`, the sharpest instance there and the *worst* one here — every functional
vanishes and the conclusion reads `0 ≤ 0`. `spidloop_tracks_exact_quarter` is built at
`λ = 1/2, −1/2, 1/4` instead: distinct, none of them `0` or `1`, every modulus at most `1/2`, so
the bound genuinely contracts. Every gain is a real Q16.16 bit vector and every fraction is
expressed through `ulp` rather than by division, the whole design generated by `q + q + q + q = 1`
together with `4q² = q`.

### The last damping gap, closed

`ThreeStateQuadTracking` handles the remaining case: one real eigenvalue `r` and a conjugate pair
`σ ± iω`, for which two of the three real functionals do not exist. The state space splits into a
real eigendirection and a real-Jordan block, and the measure splits the same way:

```
n3 = f²  +  (g₁² + g₂²)
```

One step multiplies the first term by exactly `r²` and the second by exactly `σ²+ω²`. **Both are
equalities and both are ring identities** (`n3_split`), so a step of the loop costs no inequality
of its own — the same reason the two-dimensional version avoided Cauchy–Schwarz and square roots.
The only inequality in the final bound comes from the cross-term split, which is again a sum of
squares with `α·β = 1` rather than Cauchy–Schwarz.

**Forced and free a third time, and this time checked first.** All nine relations are ring
identities in `r, σ, ω` once the gains are written through the characteristic equation in its
factored real form. The estimate for the real case was made by eyeballing the problem and was
wrong; this one was verified symbolically before any prose was written, which is that correction
put into practice rather than merely recorded.

**When it is a norm.** The three functionals have determinant
`ω · r · (r−1) · (σ²+ω²) · ((r−σ)²+ω²) · ((σ−1)²+ω²)`, and the last two factors cannot vanish when
`ω ≠ 0`. So `n3` is a norm exactly when `ω ≠ 0` and `r` is neither `0` nor `1` — the same two
excluded values as the real case, with `ω ≠ 0` standing in for distinctness. `ω = 0` is precisely
the real case, so the two measures are complementary rather than overlapping.

`pid_complex_contraction_specimen` answers the obvious objection about the `(1+α)` the split
costs: at `r = e⁻¹`, `σ = 0`, `ω = e⁻¹` and `α = β = 1` the factor is `2e⁻²`, below one, proved
with no numerics. Its last conjunct is `0 < e⁻¹`, which is `ω ≠ 0` in positive form — the design is
genuinely complex and not the real case in disguise.
...
model — are **axioms** here. As of 2026-09-14 the ledger pins **256 axioms**
(`lake env lean AxiomLedger.lean`: 224 `MachLib.*` plus 32 `Certcom.*`), of which
**169** form the trusted footprint of the headline theorems, and every one of those 167 is
modeled: 122 witnessed by a kernel-checked Mathlib term, 12 interpreted carrier and function
...
`monogate-lean`, which imports both Mathlib and MachLib and, for each of the 169 trusted axioms,
...
- **A grounded floating-point certificate is evidence about a computation only on the domain it is instantiated on.**
  The determinant, the sum, `pid_grounded` with all fifteen of its primitive forms, the four `LibmBudget` restatements,
  `eml_tree_grounded` at three `eml` trees and two of the three `eml_var_var` forms are instantiated, on stated domains of
  bounded finite inputs, and they rest on float-bridge axioms added and measured on 2026-09-14.
  `eml_var_var_certcom_witness_grounded` is not, and cannot be for binary64. §7, "Grounded floating-point certificates",
  says what each instance covers and why that one is out of reach.
...
- **Grounded floating-point certificates: which are instantiated, on which axioms and domains, and which are not
  ...
  for some primitives the finiteness of one float besides. `MachLib/FloatSafeDischarge.lean` splits them;
  `MachLib/FloatSafeInstances.lean`, `MachLib/GroundedPIDInstances.lean` and `MachLib/GroundedEMLInstances.lean` instantiate
  what the axioms allow. Every instance below has a specimen that discharges its domain at a concrete input, and all of them
  are pinned in `AxiomLedger.lean`'s headlines.

  - **The axioms, all owner-approved on 2026-09-14 and measured or witnessed before they were added.** The range axioms
    `real_fpfinite` (finite operands and an exact result at most `DBL_MAX` in magnitude give a finite `+`, `−` or `×`, and
    negation keeps a float finite) and `real_round_finite` (`floatOfR x` is finite for `|x| ≤ DBL_MAX`), with `u_lt_one`.
    Later the same day: the literal axioms `float_lit_1_5`, `float_lit_0_4` and `float_lit_0_05` (each PID gain's `Float`
    literal is `floatOfR` of its decimal, so `real_round_bounds` and `real_round_finite` give its value and finiteness); the
    libm finiteness axioms `real_exp_finite` (`exp` of a finite float at most `709`), `real_sinh_finite` and
    `real_cosh_finite` (at most `710` in magnitude) and `real_log_finite` (`log` of a finite positive float); and
    `u_le_half` (`u + u ≤ 1`). `tools/float_bridge/measure.py` measures every float-bridge one, with controls that must
    fail: the literal axioms against Lean's own evaluation of each literal, the finiteness axioms with their ranges widened
    (failing only beyond the measured overflow boundary, `709.7827…` for `exp` and `710.4758…` for `sinh` and `cosh`).
    `u_lt_one` and `u_le_half` are witnessed against Mathlib at `u = 2⁻⁵³`. The literal axioms are narrow on purpose:
    Lean's `Float.ofScientific` truncates to 64 bits before it rounds, and the generic statement, that every decimal
    literal is `floatOfR` of its decimal, fails at 81 210 of 631 326 decimals.
    Last that day: `u_le_inv_two_pow_52` (`u ≤ 2⁻⁵²`, witnessed at `u = 2⁻⁵³`) and `real_abs_eps_eq_zero` (the runtime `abs`
    is exact: measured over the finite doubles `real_abs_rounds` is measured on, the largest error is exactly `0` and every
    result is its input with the sign bit cleared).
    Now nothing but `real_fpfinite` and `real_round_finite` concludes that a float is finite from premises that assume none,
    and no axiom but those two and `real_exp_finite`, `real_sinh_finite`, `real_cosh_finite` and `real_log_finite` concludes
    that a float is finite; `no-other-float-finiteness-producer` in `tools/absence_claims.json` scans the environment and
    fires if either changes.
  - **Restated the same day: `real_log_rounds` was false, and eight rows admitted non-finite inputs.** Measured with inputs
    whose `log` lies just above a power of two (`x = e^(±2ʲ)`), which the harness had not tried, `real_log_rounds` at `u`
    fails at 20 inputs, at most `1.0162u` (glibc's `log` bounds its own error at about `0.52` ulp); it is restated at `2u`,
    where the maximum is `0.508` of the bound, and the old statement is kept as a control that must fail. `real_log_rounds`,
    `real_sqrt_rounds`, `real_asin_rounds`, `real_acos_rounds` and the `sin`, `cos`, `atan` and `abs` rounding rows also
    quantified over `±∞` and NaN, where `realToR` has no value. None was false there on its own, but on glibc's values at
    `+∞` (`fabs` and `log` return `+∞`; `asin` and `acos` return one NaN) they forced `realToR (+∞) ≤ −1`, and beside
    `real_abs_eps = 0` they had no reading at all: `abs_exact_inconsistent_with_unrestricted_forms`
    (`MachLib/FloatBridgeNonFinite.lean`) derives `False` from their old statements and those three runtime equations,
    which Lean cannot prove. All eight take `a.isFinite = true` now, which only narrows them, and
    `tools/float_bridge/measure.py` refuses to read a statement that admits a non-finite input.
  - **Instantiated: `pipeline_det_grounded`**, as `pipeline_det_grounded_instantiated`, on the domain "each of the four
    ...
    A second specimen, `pipeline_det_grounded_nonzero_specimen`, takes `α = 0.5`, `B = 10`, `x = y = floatOfR 5.0` and
    `z = w = floatOfR 1.0`: both exact products are at least `DBL_MIN`, so `ProductsNormal` is met through its `DBL_MIN`
    branch, and the exact determinant is positive. That branch needs `u + u ≤ 1`, because every lower bound on a nonzero
    float's real value carries a factor `1 − u`.
  ...
  - **Instantiated: `pid_grounded` and fourteen of the fifteen `pid_<prim>_grounded`** (the fifteenth, log-cosh, is below), on
    `PIDInputDomain env B δ`: "the
    channels `e`, `i`, `d` are finite floats whose real values are at most `B` in magnitude and each `0` or at least `δ` in
    magnitude, with `DBL_MIN ≤ 0.025·δ` and `60·B ≤ DBL_MAX`". There `FloatSafe` of the PID law holds and its computed and
    exact values are at most `60·B`. `pid_grounded_instantiated`, `pid_tanh_grounded_instantiated`,
    `pid_sin_grounded_instantiated`, `pid_cos_grounded_instantiated`, `pid_atan_grounded_instantiated` and
    `pid_abs_grounded_instantiated` need nothing more. `pid_exp_grounded_instantiated` takes `R` with `60·B ≤ R ≤ 500`;
    `pid_sinh_grounded_instantiated` and `pid_cosh_grounded_instantiated` `60·B ≤ R ≤ 710`;
    `pid_asin_grounded_instantiated` and `pid_acos_grounded_instantiated` `60·B ≤ R < 1`; and
    `pid_tan_grounded_instantiated` `60·B ≤ R ≤ 0.5`. `pid_log_grounded_instantiated`, `pid_sqrt_grounded_instantiated`
    and `pid_log10_grounded_instantiated` take `PIDPositiveDomain`, every channel at least `δ > 0`, on `[0.0125·δ, 60·B]`.
    Every one has a specimen at `pidSpecimenEnv`, where every input is `floatOfR 0.004` (in the positive domain at
    `B = 0.008` and `δ = 0.002`, with `R = 0.5`); the inputs are nonzero, so each `gain × input` product is met through the
    `DBL_MIN` branch, and `pid_grounded_specimen` shows that the exact PID law there is positive.
  - **Instantiated: `pid_log_cosh_grounded`**, as `pid_log_cosh_grounded_instantiated`, on `PIDInputDomain` with a range
    `60·B ≤ R ≤ 20`, at `lo = 1 − 4u·cosh R` and `hi = cosh R + 4u·cosh R` for the runtime `cosh`'s value read back. `lo > 0`
    is where `u ≤ 2⁻⁵²` is used: `cosh R ≤ exp R ≤ 4²⁰`, so `4u·cosh R < 1`. Its conclusion is the certificate's own, that some
    absolute bound holds. Specimen `pid_log_cosh_grounded_specimen`, at `pidSpecimenEnv` and `R = 0.5`.
  - **Instantiated: the `LibmBudget` restatements**, as `pid_sin_at_budget_instantiated`, `pid_cos_at_budget_instantiated`,
    `pid_atan_at_budget_instantiated` and `pid_abs_at_budget_instantiated`, on `PIDInputDomain`, with
    `RuntimeLibmBudget` at `B = real_sin_eps + real_cos_eps + real_atan_eps`. The `abs` one has content the flat certificate
    lacks: the runtime `abs` of the PID law reads back within `absErr` of the exact `abs`, with no libm term. The other three
    are weaker than `pid_sin_grounded_instantiated` and its siblings, because no axiom bounds those constants by a number
    and the budget is their sum. Each has a specimen at `pidSpecimenEnv`, among them `pid_abs_at_budget_specimen`.
  - **Instantiated: `eml_tree_grounded` at `eml` nodes, and two of the `eml_var_var` family.**
    `eml_tree_grounded_eml_var_var_instantiated` (`exp x − log x`) and `eml_tree_grounded_eml_const_one_instantiated`
    (`exp 1 − log x`, the constant quantized by `floatOfR`) on "`x` is a finite float with `0.5 ≤ realToR x ≤ 500`";
    `eml_tree_grounded_depth2_instantiated` (`exp(exp x − log x) − log x`) on `0.5 ≤ realToR x ≤ 2`;
    `eml_var_var_pipeline_uniform_instantiated` on "`x` a finite float in `[A, B]` with `1 ≤ A` and `B ≤ 500`"; and
    `eml_var_var_quantized_pointwise_instantiated` on "a real `x` in `(A, B)` with `0 ≤ B ≤ 200` and `2 + B ≤ 2A`". Their
    specimens take `x = floatOfR 2.0` or `floatOfR 1.0`, or `x = 3.5` in `(3.0, 4.0)`. The `exp − log` difference needs an
    upper bound on `exp`, which no trusted axiom states; `exp n ≤ 4ⁿ` is derived from `1 + x ≤ exp x`, and it is why the
    domains stop at `500`.
  ...
  - **`FloatSafe` is two conditions, and the second needs a floor.** `FloatSafe` is exactly `FloatFinite` (the computed
    floats are finite) together with `ProductsNormal` (each exact product is `0` or at least `DBL_MIN`), and the second half
    follows from a magnitude floor on the operands (`productsNormal_detEML`, `productsNormal_pidRawEML`). A bound alone is
    not enough, and that is proved: `bounded_inputs_can_have_subnormal_product`. Every lower bound on a nonzero float's real
    value carries a factor `1 − u`, so on `u < 1` alone the determinant specimen could meet the floor only through the zero
    branch; with `u + u ≤ 1`, `pipeline_det_grounded_nonzero_specimen` and every PID specimen meet it through the `DBL_MIN`
    branch.
  - **Not instantiated, and not instantiable for binary64: `eml_var_var_certcom_witness_grounded`.** Its `hMB` asks
    `ext (combinedBoundE … + 1) < B`, and the encoder's zero-count bound at `eml var var` is `14`
    (`emlVarVar_combinedBoundE_eq`, by `decide`), so `B > π/2 + 15π`, about `48.7`. Every term of the error its `hδε` asks
    to be below `ε < 1` is non-negative there, so that error is at least `u·exp B`, about `1.5·10⁵` at `u = 2⁻⁵³`. The
    obstruction is not a missing upper bound on `u`: `u ≤ 2⁻⁵²` was approved in the expectation that it would close this,
    and it does not. It would need a unit roundoff below about `7·10⁻²²`. Computed, not proved in Lean, which has no lower
    bound on `u` and so can neither instantiate nor refute the hypotheses.
