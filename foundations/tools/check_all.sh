...
if [ "${1:-}" = "--selftest" ]; then
  ...
  # Seeded through an env test seam: exercises the PARSER and the core-subset DECISION
  # without a Lean run. Producing that axiom line is the expensive half; reading it is the
  ...
  # 294-file compile pass that made this gate carry no specimen until 2026-09-04.
  run "discovered-selftest" env SELFTEST=1 bash scripts/check_discovered_compiles.sh
  ...
  # The canaries cost ~8s; `--self-test` looked unwireable only because it ALSO re-runs the full
  # 506-claim audit that the gate above already performs. `--self-test-only` is the canaries alone.
  run "claims-selftest" python3 tools/claim_audit/claim_audit.py --self-test-only
